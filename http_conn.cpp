...
const char* error_500_title = "Tnternal Error";
...
http_conn::HTTP_CODE http_conn::parse_headers(char* text)
{
	...
	if (text[0] == '\0')
	{
		/*如果HTTP请求有消息体，还需要读取m_content_length字节的消息体，状态机转移到
...
bool http_conn::write()
{
	...
	while (1)
	{
		...
		if (temp <= -1)
		{
			/*如果TCP写缓冲没哟空间，则等待下一轮EPOLLOUT事件，虽然在此期间，
			服务器无法立即收到同一客户的下一个请求， 但这可以保证连接的完整性*/
			if (errno == EAGAIN)
			{
